...
final class MetaAgeApi
{
    ...
    static function getPubKey($request)
    {
        ...
        if (!$public_key) {
            return new WP_Error('not_found', __('Public key not found!', AGE_PLUGIN), ['status' => 404]);
        }
    ...
    static function setupKeypair($force = false)
    {
        ...
        if (!function_exists('openssl_pkey_new')) {
            throw new Exception(__('OpenSSL extension is not installed!', AGE_PLUGIN));
        } else {
            ...
            if (!$rsa_key) {
                throw new Exception(sprintf(__('Unable to setup the private key. %s. Please try activating the plugin again!', AGE_PLUGIN), openssl_error_string()));
            ...
            if (!update_option('meta_public_key', trim($public_key)) || !update_option('meta_private_key', trim($private_key))) {
                throw new Exception(__('Failed to update credentials!', AGE_PLUGIN));
            }
        }
    }
    ...
    {
        $transient = get_transient('meta_age_token'); //Get token transient if exist
        ...
        if ($transient == false) {
            ...
            set_transient('meta_age_token', $token, 60 * MINUTE_IN_SECONDS); //Set 60 minute token transient
